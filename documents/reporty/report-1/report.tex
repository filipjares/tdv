...
\section{Řídký mrak bodů}
	...
	následujícím kroku použity pro odhad epipolární geometrie pří\-slu\-šných kamer.  V řádcích tabulky
	\ref{tabWbs_features} je pro každý obrázek $i$ uveden počet deskriptorů $N_i$ a také odkaz na dva
	obrázky, $j$ resp. $k$, se kterými tvoří obrázek $i$ pár s nejmenším resp. největším počtem
	předběžných korespondencí.
