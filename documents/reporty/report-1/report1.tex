...
trojrozměrný digitální model této scény. Práce ještě není dokončená, proto v této verzi zpráva
popisuje pouze hotovou část řešení.
...
kalibrační matice $\matrix K$, která má obecně pět stupňů volnosti. U fotoaparátu jsme předpokládali
...
takových úběžníků $\vector x_i$, $\vector x_j$ vyplývá jedna podmínka na obraz absolutní kuželosečky
$\vector \omega$:
	\begin{equation} \vector x_i\T \vector \omega \vector x_j. \end{equation}
Kalibrační matici jsme určili z obrazu absolutní kuželosečky na základě vztahu
	\begin{equation} \vector \omega = (\matrix K \matrix K\T)^{-1} \end{equation}
